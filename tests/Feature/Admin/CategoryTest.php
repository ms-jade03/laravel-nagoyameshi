...
class CategoryTest extends TestCase
{
    ...
    {
        $category = Category::factory()->make(['name' => 'テスト',])->toArray();
    ...
    public function test_admin_can_access_admin_categories_store()
    {
        ...
        //　ファクトリの戻り値はコレクション型となっているため、配列に変換
        $category = Category::factory()->create();
        //Category::factory()->create();
        Log::info(Category::find(1));
        //->toArray()
        Log::info($category);
    ...
    public function test_guest_cannot_access_admin_categories_update()
    {
        ...
        $response = $this->patch(route('admin.categories.update', $category), $category_update);
        $this->assertDatabaseMissing('categories', $category_update);
        $response->assertRedirect(route('admin.login'));
    }
    ...
    public function test_user_cannot_access_admin_categories_update()
    {
        ...
        $response = $this->actingAs($user)->patch(route('admin.categories.update', $category), $category_update);
        $this->assertDatabaseMissing('categories', $category_update);
        $response->assertRedirect(route('admin.login'));
    }
    ...
    public function test_admin_can_access_admin_categories_update()
    {
        ...
        $response = $this->actingAs($admin, 'admin')->patch(route('admin.categories.update', $category), $category_update);
